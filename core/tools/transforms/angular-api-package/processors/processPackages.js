  ...
  return {
    ...
    $process(docs) {
      ...
      docs.forEach(doc => {
        if (doc.docType === 'module') {
          ...
          if (doc.exports) {
            ...
            doc.structures =
                publicExports.filter(doc => doc.docType === 'enum' || doc.docType === 'interface')
                    .sort(byId);
            ...
            doc.types =
                publicExports.filter(doc => doc.docType === 'type-alias' || doc.docType === 'const')
                    .sort(byId);
          ...
          doc.packageInfo = packageMap[primaryPackageName] =
              packageMap[primaryPackageName] || {primary: undefined, secondary: []};
      ...
      Object.keys(packageMap).forEach(key => {
        ...
        pkg.primary.packageDeprecated = pkg.primary.deprecated !== undefined &&
            pkg.secondary.every(entryPoint => entryPoint.deprecated !== undefined);
      });
    }
  };
};
