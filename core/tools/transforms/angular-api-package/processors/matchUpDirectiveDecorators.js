        ...
        member.decorators.forEach(function(decorator) {
          if (decorator.name === bindingType) {
            bindings[member.name] =
                createBindingInfo(bindingType, member.name, decorator.arguments[0] || member.name);
          }
        });
      }
      // Attach binding info to the member
      // Note: this may have come from the `@Directive` decorator or from a property decorator such
      // as `@Input`.
...
function parseBinding(bindingType, option) {
  // Directive decorator bindings are of the form: "propName : bindingName" (bindingName is
  // optional)
