...
describe('getDocFromAlias', () => {
  ...
  it('should filter ambiguous docs by calling each disambiguator', () => {
    ...
    // doc2 gets removed as it has no X; then we have only one doc left so second disambiguator
    // never runs
    expect(getDocFromAlias('b')).toEqual([doc1]);
    // doc1 gets removed as it has no Y; then we have only one doc left (which would anyway pass 2nd
    // disambiguator)
