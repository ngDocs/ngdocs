 ...
 *
 * @property {boolean} relativeLinks Whether we expect the links to be relative to the originating
 * doc
 */
module.exports = function getLinkInfo(getDocFromAlias, encodeCodeBlock, log) {
  ...
  function getLinkInfoImpl(url, title, currentDoc) {
    ...
    if (!getLinkInfoImpl.useFirstAmbiguousLink && docs.length > 1) {
      ...
      linkInfo.title =
          title || docs[0].title || docs[0].name && encodeCodeBlock(docs[0].name, true);
      linkInfo.type = 'doc';
