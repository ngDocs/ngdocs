 ...
 * @description
 * Process inline link tags (of the form {@link some/uri Some Title}), replacing them with HTML
 * anchors
 * @kind function
 ...
 *
 * @property {boolean} failOnBadLink Whether to throw an error (aborting the processing) if a link
 * is invalid.
 */
  ...
  return {
    ...
    handler(doc, tagName, tagDescription) {
      ...
      return tagDescription.replace(INLINE_LINK, (match, uri, title) => {
        ...
        if (!linkInfo.valid) {
          const message =
              createDocMessage(`Error in {@${tagName} ${tagDescription}} - ${linkInfo.error}`, doc);
