...
describe('link inline-tag-def', function() {
  ...
  beforeEach(() => {
    ...
    const testPackage =
        testPackageFactory('links-package', true).factory('getLinkInfo', function() {
          return getLinkInfo;
        });
  ...
  it('should throw an error if failOnBadLink and the link is "bad"', () => {
    ...
    expect(() => tag.handler(doc, tagName, tagDescription))
        .toThrowError('Error in {@link doc-id link text} - Error message - doc');
  });
});
