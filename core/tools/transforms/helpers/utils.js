module.exports = {
  ...
  parseAttributes(str) {
    ...
    function takeValue(quote) {
      ...
      if (quote) {
        ...
        if (index >= str.length) {
          throw new Error(`Unterminated quoted attribute value in \`${str}\`. Starting at ${
              startIndex}. Expected a ${quote} but got "end of string".`);
        }
      } else {
        while (index < str.length && /\S/.test(str[index])) {
          index++;
  ...
  renderAttributes(attrMap) {
    return Object.keys(attrMap)
        .map(
            key => attrMap[key] === false ?
                '' :
                attrMap[key] === true ? ` ${key}` :
                                        ` ${key}="${attrMap[key].replace(/"/g, '&quot;')}"`)
        .join('');
  },
