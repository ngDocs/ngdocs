module.exports = function getExampleRegion(exampleMap, createDocMessage, collectExamples, log) {
  return function getExampleRegionImpl(doc, relativePath, regionName) {
    ...
    EXAMPLES_FOLDERS.some(EXAMPLES_FOLDER => {
      return exampleFile = exampleMap[EXAMPLES_FOLDER][relativePath + '.annotated'];
    });
    ...
    if (!exampleFile) {
      ...
      if (gitIgnoreFile) {
        const message =
            createDocMessage('Ignored example file... relativePath: "' + relativePath + '"', doc) +
            '\n' +
    ...
    if (!sourceCodeDoc) {
      const message = createDocMessage(
                          'Missing example region... relativePath: "' + relativePath +
                              '", region: "' + regionName + '".',
                          doc) +
          '\n' +
