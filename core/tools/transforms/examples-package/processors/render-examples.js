...
/**
 * Search the renderedContent looking for code examples that have a path (and optionally a region)
 * attribute. When they are found replace their content with the appropriate doc-region parsed
 * previously from an example file.
 */
                ...
                if (attrMap.hasOwnProperty('title')) {
                  titleVsHeaderErrors.push(createDocMessage(
                      `Using the "title" attribute for specifying a ${
                          openingTag} header is no longer supported. ` +
                ...
                if (attrMap.path) {
                  try {
                    if (closingTag !== openingTag) {
                      // The markdown renderer will wrap what it thinks is a paragraph in `<p>` and
                      // `</p>` tags. If you do not leave a blank line between a paragraph of text
                      // and a `<code-example>` then the markdown renderer may add a paragraph
                      // marker "in-between" the opening and closing tags of the code-example. For
                      // example:
                      //
                      // ```
                      // Some paragraph
