  ...
  function inlineTagDefs() {
    ...
    function tokenizeInlineTag(eat, value, silent) {
      ...
      if (match) {
        ...
        return eat(match[0])({'type': 'inlineTag', 'value': match[0]});
      }
    }
  ...
  function plainHTMLBlocks() {
    ...
    function tokenizePlainHTMLBlocks(eat, value, silent) {
      ...
      if (openMatch) {
        ...
        try {
          const fullMatch = matchRecursiveRegExp(
              value, createOpenMatcher(blockName), createCloseMatcher(blockName))[0];
          if (silent || !fullMatch) {
            // either we are not eating (silent) or the match failed
            return !!fullMatch;
          }
          return eat(fullMatch[0])({type: 'html', value: fullMatch[0]});
        } catch (e) {
