...
describe('generateKeywords processor', () => {
  ...
  it('should ignore internal and private exports', async () => {
    ...
    expect(docs[docs.length - 1].data.pages).toEqual([jasmine.objectContaining(
        {title: 'PublicExport', type: 'class'})]);
  });
       ...
       expect(docs[docs.length - 1].data).toEqual({
         ...
         pages: [
           ...
           jasmine.objectContaining(
               {title: 'BarInterface', type: 'interface', keywords: [1, 2, 3]}),
         ],
       });
     });
  ...
  it('should use `doc.searchTitle` as the title in the search index', async () => {
    ...
    expect(keywordsDoc.data.pages).toEqual([jasmine.objectContaining(
        {title: 'class PublicExport', type: 'class'})]);
  });
  ...
  it('should add member doc properties to the search terms', async () => {
    ...
    expect(keywordsDoc.data).toEqual({
      ...
      pages: [jasmine.objectContaining({members: [4, 5, 6, 7, 8, 9, 10, 11]})]
    });
  });
  ...
  it('should add inherited member doc properties to the search terms', async () => {
    ...
    expect(keywordsDoc.data).toEqual({
      dictionary:
          'class child childmember1 childmember2 parentmember1 parentmember2 parentmember3 parentclass interfac parentinterfac',
      pages: [
        jasmine.objectContaining({title: 'Child', members: [2, 3, 4, 5, 6]}),
        jasmine.objectContaining({title: 'ParentClass', members: [4, 5]}),
        jasmine.objectContaining({title: 'ParentInterface', members: [6]})
      ]
    });
  });
  ...
  it('should include both stripped and unstripped "ng" prefixed tokens', async () => {
    ...
    expect(keywordsDoc.data).toEqual({
      ...
      pages: [jasmine.objectContaining({
        headings: [6, 7],
