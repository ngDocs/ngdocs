  ...
  return {
    ...
    async $process(docs) {
      ...
      for (const doc of filteredDocs) {
        ...
        doc.searchTitle =
            doc.searchTitle || doc.title || doc.vFile && doc.vFile.title || doc.name || '';
      ...
      function tokenize(text, ignoreWords, dictionary) {
        // Split on whitespace and things that are likely to be HTML tags (this is not exhaustive
        // but reduces the unwanted tokens that are indexed).
        const rawTokens = text.split(new RegExp(
            '[\\s/]+' +                                    // whitespace
                '|' +                                      // or
                '</?[a-z]+(?:\\s+\\w+(?:="[^"]+")?)*/?>',  // simple HTML tags (e.g. <td>, <hr/>,
                                                           // </table>, etc.)
            'ig'));
        const tokens = [];
        for (let token of rawTokens) {
          ...
          // Tokens can contain letters, numbers, underscore, dot or hyphen but not at the start or
          // end. The leading TRIVIA_CHARS will capture any leading `.`, '-`' or `_` so we don't
          // have to avoid them in this regular expression. But we do need to ensure we don't
          // capture the at the end of the token.
          const POSSIBLE_TOKEN = '[a-z0-9_.-]*[a-z0-9]';
          token = token.replace(
              new RegExp(`^${TRIVIA_CHARS}*(${POSSIBLE_TOKEN})${TRIVIA_CHARS}*$`, 'i'), '$1');
      ...
      function extractMemberTokens(doc, ignoreWords, dictionary) {
        ...
        if (doc.members) {
          doc.members.forEach(
              member => memberContent.push(...tokenize(member.name, ignoreWords, dictionary)));
        }
        if (doc.statics) {
          doc.statics.forEach(
              member => memberContent.push(...tokenize(member.name, ignoreWords, dictionary)));
        ...
        if (doc.extendsClauses) {
          doc.extendsClauses.forEach(
              clause =>
                  memberContent.push(...extractMemberTokens(clause.doc, ignoreWords, dictionary)));
        }
        if (doc.implementsClauses) {
          doc.implementsClauses.forEach(
              clause =>
                  memberContent.push(...extractMemberTokens(clause.doc, ignoreWords, dictionary)));
        }
