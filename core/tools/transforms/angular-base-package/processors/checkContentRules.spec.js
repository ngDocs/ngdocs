...
describe('checkContentRules processor', function() {
  ...
  it('should run configured rules against matching docs', () => {
    ...
    processor.docTypeRules = {
      'test1': {name: [nameSpy1, nameSpy3], description: [descriptionSpy1, descriptionSpy3]},
      'test2': {name: [nameSpy2], description: [descriptionSpy2]}
    };
  ...
  it('should log warnings if the rule returns error messages and `failOnContentErrors` is false',
     () => {
       ...
       const descriptionSpy1 =
           jasmine.createSpy('description 1').and.returnValue('description error message');

       processor.failOnContentErrors = false;
       processor.docTypeRules = {'test1': {name: [nameSpy1], description: [descriptionSpy1]}};
