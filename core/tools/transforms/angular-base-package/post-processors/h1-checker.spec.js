...
describe('h1Checker postprocessor', () => {
  ...
  it('should complain if there is more than one h1 in a document', () => {
    ...
    expect(() => processor.$process([doc]))
        .toThrowError(createDocMessage('More than one h1 found in ' + doc.renderedContent, doc));
  });
  ...
  it('should attach the h1 text to the vFile', () => {
    const doc = {docType: 'a', renderedContent: '<h1>Heading 1</h1>'};
  ...
  it('should clean aria-hidden anchors from h1 text added to the vFile', () => {
    const doc = {
      docType: 'a',
      renderedContent: '<h1 class="no-toc" id="what-is-angular">' +
