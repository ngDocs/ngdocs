...
describe('NgPackagesInstaller', () => {
  ...
  describe('checkDependencies()', () => {
    ...
    it('should not print a warning if there is no _local_.json file', () => {
      ...
      expect(fs.existsSync)
          .toHaveBeenCalledWith(path.resolve(projectDir, 'node_modules/_local_.json'));
  ...
  describe('installLocalDependencies()', () => {
    ...
    let dummyLocalPackages, dummyPackage, dummyPackageJson, expectedModifiedPackage,
        expectedModifiedPackageJson;
    ...
    describe('when there is no local package marker', () => {
      ...
      beforeEach(() => {
        ...
        installer._installDeps.and.callFake(
            (...args) => log.push(`installDeps: ${args.join(' ')}`));
      ...
      it('should temporarily overwrite the package.json files of local Angular packages', () => {
        ...
        const overwriteConfigFor = (pkgName, newProps) =>
            Object.assign(pkgConfigFor(pkgName), newProps);
      ...
      it('should overwrite package.json, then install deps, then restore original package.json',
         () => {
           expect(log).toEqual([
             ...
             'installDeps: --pure-lockfile --check-files', `writeFile: ${dummyPackageJson}`
           ]);
         });
  ...
  describe('_overwritePackageVersion()', () => {
    ...
    it('should overwrite the package version if it is a dependency and found in the lockfile',
       () => {
         ...
         installer._overwritePackageVersion(pkgConfig.name, pkgConfig, projectConfig, lockFile);
         expect(pkgConfig.version)
             .toBe('project-version-prod+locally-overwritten-by-ngPackagesInstaller');
         ...
         installer._overwritePackageVersion(pkgConfig.name, pkgConfig, projectConfig, lockFile);
         expect(pkgConfig.version)
             .toBe('project-version-dev+locally-overwritten-by-ngPackagesInstaller');
  ...
  describe('_parseLockfile()', () => {
    ...
    it('should throw if parsing the lockfile fails', () => {
      ...
      expect(() => installer._parseLockfile('/foo/bar/yarn.lock'))
          .toThrowError(
  ...
  describe('_printWarning()', () => {
    it('should mention the message passed in the warning', () => {
      ...
      expect(console.warn.calls.argsFor(0)[0])
          .toContain('is running against the local Angular/Zone.js build');
    });
    ...
    it('should mention the command to restore the Angular packages in any warning', () => {
      ...
      const dir1 = '.';
      const restoreCmdRe1 =
          RegExp('\\bnode .*?ng-packages-installer/index restore ' + path.resolve(dir1));
      ...
      const restoreCmdRe2 =
          RegExp(`\\bnode .*?ng-packages-installer/index restore .*?${path.resolve(dir1)}\\b`);
  ...
  describe('_installDeps()', () => {
    it('should run yarn install with the given options', () => {
      ...
      expect(shelljs.exec).toHaveBeenCalledWith('yarn install option-1 option-2', {
    ...
    describe('_setLocalMarker', () => {
      it('should create a local marker file', () => {
        ...
        expect(fs.writeFileSync)
            .toHaveBeenCalledWith(path.resolve(nodeModulesDir, '_local_.json'), 'test contents');
      });
    });
  });
});
