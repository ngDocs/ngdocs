...
const DIST_PACKAGES_BUILD_SCRIPT =
    path.join(ANGULAR_ROOT_DIR, 'scripts/build/build-packages-dist.js');
...
class NgPackagesInstaller {
   ...
   *     * `force` (`boolean`) - whether to force a local installation even if there is a local
   * marker file.
   *     * `buildPackages` (`boolean`) - whether to build the local Angular/Zone.js packages before
   * using them. (NOTE: Building the packages is currently not supported on Windows, so a message is
   * printed instead.)
   *     * `ignorePackages` (`string[]`) - a collection of names of packages that should not be
   * copied over.
   */
  ...
  /**
   * Install locally built Angular/Zone.js dependencies, overriding the dependencies in the
   * `package.json`. This will also write a "marker" file (`_local_.json`), which contains the
   * overridden `package.json` contents and acts as an indicator that dependencies have been
   * overridden.
   */
    ...
    if (this.force || !this._checkLocalMarker()) {
      ...
      try {
        ...
        Object.keys(packages).forEach(key => {
          ...
          // Overwrite project dependencies/devDependencies to Angular/Zone.js packages with local
          // files.
          ['dependencies', 'devDependencies'].forEach(prop => {
            ...
            Object.keys(deps).forEach(key2 => {
              ...
              if (pkg2) {
                ...
                this._log(`Overriding dependency of local ${key} with local package: ${key2}: ${
                    deps[key2]}`);
              }
            });
          });
        ...
        const [dependencies, peers] =
            this._collectDependencies(packageConfig.dependencies || {}, packages);
        const [devDependencies, devPeers] =
            this._collectDependencies(packageConfig.devDependencies || {}, packages);
        ...
        const localPackageConfig =
            Object.assign(Object.create(null), packageConfig, {dependencies, devDependencies});
  ...
  _assignPeerDependencies(peerDependencies, dependencies, devDependencies, parsedLockfile) {
    Object.keys(peerDependencies).forEach(key => {
      ...
      // If there is already an equivalent dependency then override it - otherwise assign/override
      // the devDependency
      if (dependencies[key]) {
        ...
        this._log(
            `${
                devDependencies[key] ? 'Overriding' :
                                       'Assigning'} devDependency with peerDependency: ` +
            `${key}: ${peerDepRange}`);
   ...
   *
   * NOTE:
   * Building the packages is currently not supported on Windows, so a message is printed instead,
   * prompting the user to do it themselves (e.g. using Windows Subsystem for Linux or a docker
   * container).
   */
  _buildDistPackages() {
    ...
    if (canBuild) {
      ...
      this._warn([
        ...
        `Please, ensure '${ANGULAR_DIST_PACKAGES_DIR}', '${AIMWA_DIST_PACKAGES_DIR}' and ` +
            `'${ZONEJS_DIST_PACKAGES_DIR}' exist and are up-to-date (e.g. by running '${
                DIST_PACKAGES_BUILD_SCRIPT}' ` +
   ...
   * (Detected as directories in '/dist/packages-dist/' and '/dist/zone.js-dist/' that contain a
  ...
  _getDistPackages() {
    ...
    this._log(
        'Found the following Angular distributables:',
        ...Object.keys(packageConfigs).map(key => `\n - ${key}`));
  ...
  /**
   * Update a package's version with the fake version based on the package's original version in the
   * projects's lockfile.
   *
   * **Background:**
   * This helps avoid version mismatch errors with the CLI.
   * Since the version set by bazel on the locally built packages is determined based on the latest
   * tag for a commit on the current branch, it is often the case that this version is older than
   * what the current `@angular/cli` version is compatible with (e.g. if the user has not fetched
   * the latest tags from `angular/angular` or the branch has not been rebased recently.
   *
   ...
   * @param {import('@yarnpkg/lockfile').LockFileObject} projectLockfile - The projects's parsed
   *     `yarn.lock`.
   */
  _overwritePackageVersion(packageName, packageConfig, projectConfig, projectLockfile) {
    ...
    if (projectVersion !== undefined) {
      ...
      this._log(`Overwriting the version of '${packageName}': ${packageConfig.version} --> ${
          newVersion}`);
  ...
  /**
   * Extract the value for a boolean cli argument/option. When passing an option multiple times,
   * `yargs` parses it as an array of boolean values. In that case, we only care about the last
   * occurrence.
   *
   * This can be useful, for example, when one has a base command with the option turned on and
   * another command (building on top of the first one) turning the option off:
   * ```
  ...
  _parseLockfile(lockfilePath) {
    ...
    if (parsed.type !== 'success') {
      throw new Error(`[${NgPackagesInstaller.name}]: Error parsing lockfile '${
          lockfilePath}' (result type: ${parsed.type}).`);
    }
...
function main() {
  ...
  yargs
      ...
      .option(
          'force', {describe: 'Force the command to execute even if not needed.', default: false})
      .option('build-packages', {
        describe: 'Build the local Angular/Zone.js packages, before using them.',
        default: false
      })
      .option('ignore-packages', {
        describe: 'List of Angular/Zone.js packages that should not be used in local mode.',
        default: [],
        array: true
      })

      .command(
          'overwrite <projectDir> [--force] [--debug] [--ignore-packages package1 package2]',
          'Install dependencies from the locally built Angular/Zone.js distributables.', () => {},
          argv => {
            createInstaller(argv).installLocalDependencies();
          })
      .command(
          'restore <projectDir> [--debug]', 'Install dependencies from the npm registry.', () => {},
          argv => {
            createInstaller(argv).restoreNpmDependencies();
          })
      .command(
          'check <projectDir> [--debug]',
          'Check that dependencies came from npm. Otherwise display a warning message.', () => {},
          argv => {
