...
describe('example-boilerplate tool', () => {
  describe('add', () => {
    ...
    it('should run `ngcc`', () => {
      ...
      expect(shelljs.exec)
          .toHaveBeenCalledWith(`yarn --cwd ${sharedDir} ngcc --properties es2015 main`);
    });
    ...
    it('should symlink the node_modules', () => {
      ...
      expect(fs.ensureSymlinkSync)
          .toHaveBeenCalledWith(sharedNodeModulesDir, path.resolve('a/b/node_modules'));
      expect(fs.ensureSymlinkSync)
          .toHaveBeenCalledWith(sharedNodeModulesDir, path.resolve('c/d/node_modules'));
    });

    it('should error if the node_modules folder is missing', () => {
      fs.existsSync.and.returnValue(false);
      expect(() => exampleBoilerPlate.add())
          .toThrowError(
              `The shared node_modules folder for the examples (${
                  sharedNodeModulesDir}) is missing.\n` +
    ...
    it('should not copy boilerplate files that are match `overrideBoilerplate` in the example-config.json file',
       () => {
         const boilerplateDir = path.resolve(sharedDir, 'boilerplate');
         exampleBoilerPlate.loadJsonFile.and.returnValue({'overrideBoilerplate': ['c/d']});
    ...
    it('should try to load the example config file', () => {
      ...
      expect(exampleBoilerPlate.loadJsonFile)
          .toHaveBeenCalledWith(path.resolve('a/b/example-config.json'));
      expect(exampleBoilerPlate.loadJsonFile)
          .toHaveBeenCalledWith(path.resolve('c/d/example-config.json'));
    });
  });
  ...
  describe('remove', () => {
    it('should run `git clean`', () => {
      ...
      expect(shelljs.exec).toHaveBeenCalledWith('git clean -xdfq', {
        cwd: path.resolve(__dirname, '../../content/examples')
      });
    });
  });
  ...
  describe('listOverrides', () => {
    ...
    it('should list all files that are overridden in examples', () => {
      spyOn(exampleBoilerPlate, 'loadJsonFile')
          .and.returnValues(
              {
                'overrideBoilerplate': ['angular.json', 'tsconfig.json']
      ...
      expect(exampleBoilerPlate.getFoldersContaining)
          .toHaveBeenCalledWith(examplesDir, 'example-config.json', 'node_modules');
      expect(console.log)
          .toHaveBeenCalledWith('Boilerplate files that have been overridden in examples:');
    ...
    it('should display a helpful message if there are no overridden files', () => {
      ...
      expect(exampleBoilerPlate.getFoldersContaining)
          .toHaveBeenCalledWith(examplesDir, 'example-config.json', 'node_modules');
      expect(console.log)
          .toHaveBeenCalledWith('No boilerplate files have been overridden in examples.');
  ...
  describe('getFoldersContaining', () => {
    it('should use glob.sync', () => {
      ...
      const result =
          exampleBoilerPlate.getFoldersContaining('base/path', 'config.json', 'node_modules');
      expect(glob.sync).toHaveBeenCalledWith(
          path.resolve('base/path/**/config.json'),
