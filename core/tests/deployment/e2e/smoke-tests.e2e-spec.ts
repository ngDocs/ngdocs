  ...
  describe('(smoke tests)', () => {
    ...
    describe('(marketing pages)', () => {
      ...
      Object.keys(textPerUrl).forEach(url => {
        it(`should show the page at '${url}'`, async () => {
          await page.goTo(url);
          await browser.wait(
              () => page.getDocViewerText(), 5000);  // Wait for the document to be loaded.
    ...
    describe('(api docs pages)', () => {
      const textPerUrl: {[key: string]: string} = {
        ...
        /* Const */
        'api/forms/NG_VALIDATORS': 'const ng_validators',
        /* Decorator */
        'api/core/Component': '@component',
        /* Directive */
        'api/common/NgIf': 'class ngif',
        /* Enum */
        'api/core/ChangeDetectionStrategy': 'enum changedetectionstrategy',
        /* Function */
        'api/animations/animate': 'animate(',
        /* Global */
        'api/core/global/ngApplyChanges': 'ng.applychanges(',
        /* Interface */
        'api/core/OnDestroy': 'interface ondestroy',
        /* Pipe */
        'api/common/JsonPipe': '| json',
        /* Type-Alias */
        'api/common/http/HttpEvent': 'type httpevent',
      };
