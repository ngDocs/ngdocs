...
export class SitePage {
  ...
  ghLinks = this.docViewer.all(by.css('a'))
  ...
  getTopMenuLink(path: string) {
    return element(by.css(`aio-top-menu a[href="${path}"]`));
  }

  ga() {
    return browser.executeScript<any[][]>('return window["ga"].q');
  }
  locationPath() {
    return browser.executeScript<string>('return document.location.pathname');
  }
