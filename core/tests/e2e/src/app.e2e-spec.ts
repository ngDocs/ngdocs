...
describe('site App', () => {
  ...
  it('should not navigate when clicking on nav-item headings (sub-menu toggles)', async () => {
    ...
    async function testNavItemHeading(heading: ElementFinder, level: number) {
      ...
      await childNavItemHeadings.each(
          childHeading => childHeading && testNavItemHeading(childHeading, nextLevel));
  ...
  it('should render `{@example}` dgeni tags as `<code-example>` elements with HTML escaped content',
     async () => {
       ...
       expect(await page.getInnerHtml(codeExample))
           .toContain('&lt;h1&gt;Tour of Heroes&lt;/h1&gt;');
     });
  ...
  describe('contributors page', () => {
    ...
    it('should have contributors listed in each group', async () => {
      ...
      const clickButton = (elementFinder: ElementFinder) =>
          browser.executeScript('arguments[0].click()', elementFinder);
  ...
  describe('404 page', () => {
    it('should add or remove the "noindex" meta tag depending upon the validity of the page',
       async () => {
         ...
         expect(await element(by.css('meta[name="robots"][content="noindex"]')).isPresent())
             .toBeTruthy();
