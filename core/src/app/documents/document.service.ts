...
export class DocumentService {
        ...
        .pipe(
            ...
            map((data: UnsafeDocumentContents) => ({
                  ...
                  contents: data.contents === null ?
                      null :
                      // SECURITY: HTML is authored by the documentation team and is fetched
                      // directly from the server
                      htmlFromStringKnownToSatisfyTypeContract(data.contents, '^')
                })),
            catchError(
                (error: HttpErrorResponse) => error.status === 404 ? this.getFileNotFoundDoc(id) :
  ...
  private getFileNotFoundDoc(id: string): Observable<DocumentContents> {
    if (id !== FILE_NOT_FOUND_ID) {
      ...
      // using `getDocument` means that we can fetch the 404 doc contents from the server and cache
      // it
      return this.getDocument(FILE_NOT_FOUND_ID);
    } else {
      return of({id: FILE_NOT_FOUND_ID, contents: htmlEscape('Document not found')});
    }
  }
