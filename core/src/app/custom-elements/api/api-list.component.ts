...
export class ApiListComponent implements OnInit {
  ...
  statuses: Option[] = [
    {value: 'all', title: 'All'}, {value: 'stable', title: 'Stable'},
    {value: 'deprecated', title: 'Deprecated'}, {value: 'security-risk', title: 'Security Risk'}
  ];
        ...
        combineLatest([
          ...
          this.criteriaSubject,
            ...
            .pipe(
                ...
                map(results => (results.sections.map(
                        section => (
                            {...section, items: this.filterSection(section, results.criteria)})))));
  ...
  private filterSection(section: ApiSection, {query, status, type}: SearchCriteria) {
    ...
    const matchesQuery = (item: ApiItem) => sectionNameMatches || item.name.indexOf(query) !== -1;
    const matchesStatus = (item: ApiItem) => status === 'all' || status === item.stability ||
        (status === 'security-risk' && item.securityRisk);
    const matchesType = (item: ApiItem) => type === 'all' || type === item.docType;

    const items: ApiItem[] =
        (section.items ||
         []).filter(item => matchesType(item) && matchesStatus(item) && matchesQuery(item));

    // If there are no items we still return an empty array if the section name matches and the type
    // is 'package'
  ...
  private initializeSearchCriteria() {
    ...
    this.searchCriteria = {query: q, status: this.status.value, type: this.type.value};
