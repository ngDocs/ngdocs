...
describe('ApiListComponent', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        {provide: ApiService, useClass: TestApiService}, {provide: Logger, useClass: MockLogger},
  ...
  function expectFilteredResult(label: string, itemTest: (item: ApiItem) => boolean) {
    component.filteredSections.subscribe(filtered => {
      ...
      expect(filtered.every(section => section.items?.every(itemTest)))
          .withContext(label)
          .toBe(true);
    });
  }
  ...
  describe('#filteredSections', () => {
    ...
    it('items should be an array for every item in section when query matches section name', () => {
      ...
      component.filteredSections.subscribe(filtered => {
        ...
        expect(filtered[0].items)
            .toEqual(sections.find(section => section.name === 'core')?.items as ApiItem[]);
      });
    });
  ...
  describe('initial criteria from location', () => {
    ...
    it('should filter as expected for ?query&status&type', () => {
      locationService.query = {query: 's_1', status: 'experimental', type: 'class'};
      fixture.detectChanges();
