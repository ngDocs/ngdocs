...
@Component({selector: 'aio-events', templateUrl: 'events.component.html'})
export class EventsComponent implements OnInit {
  ...
  ngOnInit() {
    this.eventsService.events.subscribe(events => {
      this.pastEvents =
          events.filter(event => new Date(event.date.end).getTime() < Date.now() - DAY)
  ...
  getEventDates(event: Event) {
    ...
    if (event.workshopsDate) {
      ...
      dateString = areWorkshopsBeforeEvent ? `${workshopsDateString}, ${mainEventDateString}` :
...
function processDate(dates: Duration) {
  ...
  // If they are in different months add the string '- February 2' Making the final string January
  // 31 - February 2
