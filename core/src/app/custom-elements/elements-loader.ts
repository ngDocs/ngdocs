...
export class ElementsLoader {
  ...
  constructor(
      ...
      @Inject(ELEMENT_MODULE_LOAD_CALLBACKS_TOKEN) elementModulePaths:
          Map<string, LoadChildrenCallback>, private compiler: Compiler) {
  ...
  loadContainedCustomElements(element: HTMLElement): Observable<void> {
    const unregisteredSelectors =
        Array.from(this.elementsToLoad.keys()).filter(s => element.querySelector(s));
  ...
  loadCustomElement(selector: string): Promise<void> {
    ...
    if (this.elementsToLoad.has(selector)) {
      ...
      const loadedAndRegistered =
          (modulePathLoader() as
           Promise<NgModuleFactory<WithCustomElementComponent>|Type<WithCustomElementComponent>>)
              .then(elementModuleOrFactory => {
                 ...
                 * to pass on the NgModuleFactory, or compile the NgModule and return its
                 * NgModuleFactory.
                 */
