...
export class CodeTabsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    //
    // NOTE:
    // In IE11, doing this also empties the `<code-pane>` nodes captured in `codeExamples`
    // ¯\_(ツ)_/¯ Only remove the unnecessary nodes after having captured the `<code-pane>`
    // contents.
