...
describe('CodeComponent', () => {
  ...
  describe('pretty printing', () => {
    ...
    it('should format a one-line code sample without linenums by default', () => {
      ...
      expect(getFormattedCode())
          .toBe(`Formatted code (language: auto, linenums: false): ${oneLineCode}`);
    });
    ...
    it('should add line numbers to one-line code sample when linenums is `true`', () => {
      ...
      expect(getFormattedCode())
          .toBe(`Formatted code (language: auto, linenums: true): ${oneLineCode}`);
    });
    ...
    it('should format a small multi-line code sample without linenums by default', () => {
      ...
      expect(getFormattedCode())
          .toBe(`Formatted code (language: auto, linenums: false): ${
              htmlEscape(smallMultiLineCode)}`);
    });
    ...
    it('should add line numbers to  a small multi-line code sample when linenums is `\'true\'`',
       () => {
         ...
         expect(getFormattedCode())
             .toBe(`Formatted code (language: auto, linenums: true): ${
                 htmlEscape(smallMultiLineCode)}`);
       });
    ...
    it('should add line numbers to a big multi-line code sample when linenums is `true`', () => {
      ...
      expect(getFormattedCode())
          .toBe(`Formatted code (language: auto, linenums: true): ${htmlEscape(bigMultiLineCode)}`);
    });
  ...
  describe('error message', () => {
    ...
    it('should not display "code-missing" class when there is some code', () => {
      expect(getErrorMessage())
          .withContext('should not have element with "code-missing" class')
          .toBeNull();
    });
  ...
  describe('copy button', () => {
    ...
    it('should display an error when copy fails', () => {
      ...
      expect(logger.error.calls.mostRecent().args[0].message)
          .toMatch(/^ERROR copying code to clipboard:/);
    });
  });
});
