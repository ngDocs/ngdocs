...
describe('TocComponent', () => {
  ...
  let page: {
    listItems: DebugElement[]; tocHeading: DebugElement; tocHeadingButtonEmbedded: DebugElement;
    ...
    describe('(once the lifecycle hooks have run)', () => {
      ...
      it('should only display H2 and H3 TocItems', () => {
        tocService.tocList.next(
            [tocItem('Heading A', 'h1'), tocItem('Heading B'), tocItem('Heading C', 'h3')]);
        fixture.detectChanges();
      ...
      it('should stop listening for TocItems once destroyed', () => {
        ...
        tocService.tocList.next(
            [tocItem('Heading A', 'h1'), tocItem('Heading B'), tocItem('Heading C')]);
        fixture.detectChanges();
        expect(tocComponentDe.queryAll(By.css('li')).length).toBe(1);
      });
      ...
      describe('when fewer than `maxPrimary` TocItems', () => {
        ...
        it('should not display expando buttons', () => {
          expect(page.tocHeadingButtonEmbedded)
              .withContext('top expand/collapse button')
              .toBeFalsy();
      ...
      describe('when many TocItems', () => {
        ...
        beforeEach(() => {
          ...
          page = setPage();
          scrollToTopSpy =
              (TestBed.inject(ScrollService) as unknown as TestScrollService).scrollToTop;
        });
        ...
        it('should not display the h1 item', () => {
          expect(page.listItems.find(item => item.classes.h1))
              .withContext('should not find h1 item')
              .toBeFalsy();
        });
...
//// helpers ////
@Component({selector: 'aio-embedded-host', template: '<aio-toc class="embedded"></aio-toc>'})
class HostEmbeddedTocComponent {
}

@Component({selector: 'aio-not-embedded-host', template: '<aio-toc></aio-toc>'})
