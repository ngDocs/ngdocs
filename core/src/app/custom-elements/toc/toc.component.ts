...
@Component({selector: 'aio-toc', templateUrl: 'toc.component.html', styles: []})
export class TocComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
      private scrollService: ScrollService, elementRef: ElementRef,
  ...
  ngOnInit() {
    this.tocService.tocList.pipe(takeUntil(this.onDestroy)).subscribe(tocList => {
      ...
      this.type = (itemCount > 0) ?
          this.isEmbedded ?
          (itemCount > this.primaryMax) ? 'EmbeddedExpandable' : 'EmbeddedSimple' :
          'Floating' :
          'None';
    });
  }
  ...
  ngAfterViewInit() {
    if (!this.isEmbedded) {
      // We use the `asap` scheduler because updates to `activeItemIndex` are triggered by DOM
      // changes, which, in turn, are caused by the rendering that happened due to a
      // ChangeDetection. Without asap, we would be updating the model while still in a
      // ChangeDetection handler, which is disallowed by Angular.
      combineLatest([
