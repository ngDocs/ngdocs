...
export class CustomIconRegistry extends MatIconRegistry {
  ...
  constructor(
      http: HttpClient, sanitizer: DomSanitizer, @Optional() @Inject(DOCUMENT) document: Document,
  ...
  override getNamedSvgIcon(iconName: string, namespace?: string) {
    ...
    return preloadedElement ? of(preloadedElement.cloneNode(true) as SVGElement) :
                              super.getNamedSvgIcon(iconName, namespace);
  }

  private loadSvgElement(iconName: string, namespace?: string): SVGElement|undefined {
    const svgIcon = this.svgIcons.find(
        icon => namespace ? icon.name === iconName && icon.namespace === namespace :
                            icon.name === iconName);
