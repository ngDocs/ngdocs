...
export function getAttrs(el: HTMLElement|ElementRef): AttrMap {
  ...
  for (const attr of attrs as any as
       Attr[] /* cast due to https://github.com/Microsoft/TypeScript/issues/2695 */) {
...
export function getAttrValue(attrs: AttrMap, attr: string|string[]): string|undefined {
  const key =
      (typeof attr === 'string') ? attr : attr.find(a => attrs.hasOwnProperty(a.toLowerCase()));
...
export function getBoolFromAttribute(
    el: HTMLElement|ElementRef, attr: string|string[], def: boolean = false): boolean {
