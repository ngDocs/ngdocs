...
describe('TocService', () => {
  ...
  describe('TocItem for an h2 with links and extra whitespace', () => {
    ...
    it('should have removed anchor link and GitHub links from tocItem html content', () => {
      expect((tocItem.content as TestSafeHtml).changingThisBreaksApplicationSecurity)
          .toEqual('Setup to develop <i>locally</i>.');
    });

    it('should have bypassed HTML sanitizing of heading\'s innerHTML ', () => {
      const domSanitizer: TestDomSanitizer =
          injector.get(DomSanitizer) as unknown as TestDomSanitizer;
...
class MockScrollSpyService {
  private $$lastInfo: {active: Subject<ScrollItem|null>; unspy: jasmine.Spy;}|undefined;
