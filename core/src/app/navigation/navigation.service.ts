...
export {CurrentNode, CurrentNodes, NavigationNode, NavigationResponse, NavigationViews, VersionInfo,} from './navigation.model';
...
export class NavigationService {
  /**
   * An observable collection of NavigationNode trees, which can be used to render navigational
   * menus
   */
  ...
  constructor(private http: HttpClient, private location: LocationService) {
    ...
    // The version information is packaged inside the navigation response to save us an extra
    // request.
   ...
   * We use `publishLast` because once the http request is complete the request observable
   * completes. If you use `publish` here then the completed request observable will cause the
   * subscribed observables to complete too. We `connect` to the published observable to trigger the
   * request immediately. We could use `.refCount` here but then if the subscribers went from 1 -> 0
   * -> 1 then you would get another request to the server. We are not storing the subscription from
   * connecting as we do not expect this service to be destroyed.
   */
    ...
    const currentNodes =
        combineLatest([
          ...
          this.location.currentPath,
            ...
            .pipe(
                map((result) => ({navMap: result[0], url: result[1]})), map((result) => {
                  const matchSpecialUrls = /^api/.exec(result.url);
                  if (matchSpecialUrls) {
                    result.url = matchSpecialUrls[0];
                  }
                  return result.navMap.get(result.url) ||
                      {'': {view: '', url: result.url, nodes: []}};
                }),
                publishReplay(1));
  ...
  private computeUrlToNavNodesMap(navigation: NavigationViews) {
    ...
    Object.keys(navigation)
        .forEach(view => navigation[view].forEach(node => this.walkNodes(view, navMap, node)));
    return navMap;
  }
   ...
   */
  private walkNodes(
      view: string, navMap: Map<string, CurrentNodes>, node: NavigationNode,
