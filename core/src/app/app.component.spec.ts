...
describe('AppComponent', () => {
  ...
  async function awaitDocRendered() {
    const newDocPromise =
        new Promise(resolve => documentService.currentDocument.subscribe(resolve));
    const docRenderedPromise =
        new Promise(resolve => docViewerComponent.docRendered.subscribe(resolve));
  ...
  describe('with proper DocViewer', () => {
        ...
        describe('when manually opened', () => {
          ...
          it('should close when clicking in gray content area overlay', () => {
            const sidenavBackdrop =
                fixture.debugElement.query(By.css('.mat-drawer-backdrop')).nativeElement;
    ...
    describe('SideNav version selector', () => {
      ...
      it('should navigate when change to a version with a url', async () => {
        ...
        selectElement.triggerEventHandler(
            'change', {option: versionWithUrl, index: versionWithUrlIndex});
      ...
      it('should not navigate when change to a version without a url', async () => {
        ...
        selectElement.triggerEventHandler(
            'change', {option: versionWithoutUrl, index: versionWithoutUrlIndex});
        expect(locationService.go).not.toHaveBeenCalled();
      });

      it('should navigate when change to a version with a url that does not end with `/`',
         async () => {
           await setupSelectorForTesting();
           ...
           expect(locationService.go)
               .toHaveBeenCalledWith('https://next.angular.io/docs#section-1');
         });
    });
    ...
    describe('auto-scrolling', () => {
      ...
      it('should call `scrollAfterRender` (via `onDocInserted`) when navigate to a new Doc',
         fakeAsync(() => {
           locationService.go('guide/pipes');
           ...
           tick(500);  // there are other outstanding timers in the AppComponent that are not
                       // relevant
         }));
    });
    ...
    describe('aio-toc', () => {
      ...
      it('should not be loaded/registered until necessary', () => {
        const loader =
            fixture.debugElement.injector.get(ElementsLoader) as unknown as TestElementsLoader;
    ...
    describe('footer', () => {
      it('should have version number', () => {
        const versionEl: HTMLElement =
            fixture.debugElement.query(By.css('aio-footer')).nativeElement;
    ...
    describe('deployment banner', () => {
      it('should show a message if the deployment mode is "archive"', async () => {
        ...
        const banner: HTMLElement =
            fixture.debugElement.query(By.css('aio-mode-banner')).nativeElement;
    ...
    describe('search', () => {
      ...
      describe('click handling', () => {
        ...
        it('should clear "only" the search query param from the URL', () => {
          ...
          expect(locationService.setSearch)
              .toHaveBeenCalledWith('', {a: 'some-A', b: 'some-B', search: undefined});
        });
      ...
      describe('keyup handling', () => {
        it('should grab focus when the / key is pressed', () => {
          const searchBox: SearchBoxComponent =
              fixture.debugElement.query(By.directive(SearchBoxComponent)).componentInstance;
      ...
      describe('showing search results', () => {
        it('should not display search results when query is empty', () => {
          const searchService =
              TestBed.inject(SearchService) as Partial<SearchService>as MockSearchService;
    ...
    describe('archive redirection', () => {
      ...
      Object.keys(redirectionPerMode).forEach(mode => {
        ...
        const description =
            `should ${doRedirect ? '' : 'not '}redirect to 'docs' if deployment mode is '${
                mode}' ` +
        ...
        const verifyRedirection = () =>
            expect(TestBed.inject(LocationService).replace).toHaveBeenCalledWith('docs');
  ...
  describe('with mocked DocViewer', () => {
    ...
    beforeEach(() => {
      ...
      TestBed.overrideModule(
          AppModule,
          {remove: {declarations: [DocViewerComponent]}, add: {schemas: [NO_ERRORS_SCHEMA]}});
    });
    ...
    describe('initial rendering', () => {
      ...
      it('should initially add the starting class until a document is rendered', () => {
        ...
        const sidenavContainer =
            fixture.debugElement.query(By.css('mat-sidenav-container')).nativeElement;
    ...
    describe('hostClasses', () => {
      ...
      it('should set the css class of the host container based on the open/closed state of the side nav',
         async () => {
           ...
           async function waitForSidenavOpenedChange() {
             const promise =
                 new Promise(resolve => sidenav.openedChange.pipe(first()).subscribe(resolve));
      ...
      function checkHostClass(type: string, value: string) {
        ...
        expect(classArray.length)
            .withContext(`"${classes}" should have only one class matching ${type}-*`)
            .toBeLessThanOrEqual(1);
        expect(classArray).withContext(`"${classes}" should contain ${type}-${value}`).toEqual([
          `${type}-${value}`
        ]);
      }
    });
...
class TestElementsLoader {
  loadContainedCustomElements =
      jasmine.createSpy('loadContainedCustomElements').and.returnValue(of(undefined));
