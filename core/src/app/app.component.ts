...
export class AppComponent implements OnInit {
   ...
   *
   * * `page-...`: computed from the current document id (e.g. events, guide-security,
   * tutorial-toh-pt2)
  ...
  get mode() {
    return this.isOpened ? 'side' : 'over';
  }
  ...
  constructor(
      public deployment: Deployment, private documentService: DocumentService,
      private hostElement: ElementRef, private locationService: LocationService,
      private navigationService: NavigationService, private scrollService: ScrollService,
      private searchService: SearchService, private swUpdatesService: SwUpdatesService,
  ...
  ngOnInit() {
    ...
    combineLatest([
      ...
      this.currentDocVersion = this.docVersions.find(
                                   version => version.title === this.deployment.mode ||
                                       version.title === `v${versionInfo.major}`) as NavigationNode;
    ...
    combineLatest([
      hasNonEmptyToc, this.showFloatingToc
  ...
  @HostListener(
      'click',
      ['$event.target', '$event.button', '$event.ctrlKey', '$event.metaKey', '$event.altKey'])
  onClick(
      eventTarget: HTMLElement, button: number, ctrlKey: boolean, metaKey: boolean,
  ...
  updateHostClasses() {
    ...
    const notificationAnimatingClass =
        this.notificationAnimating ? 'aio-notification-animating' : '';

    this.hostClasses = [
      mode, sideNavOpen, pageClass, folderClass, viewClasses, notificationClass,
  ...
  onScroll() {
    ...
    this.tocMaxHeight =
        (document.body.scrollHeight - window.pageYOffset - this.tocMaxHeightOffset).toFixed(2);
  }
