...
import {FETCHING_ERROR_ID, FILE_NOT_FOUND_ID} from 'app/documents/document.service';
...
import {MockElementsLoader, MockTitle, MockTocService, ObservableWithSubscriptionSpies, TestDocViewerComponent, TestModule, TestParentComponent} from 'testing/doc-viewer-utils';
...
describe('DocViewerComponent', () => {
  ...
  describe('#doc', () => {
    ...
    const setCurrentDoc = (newDoc: TestParentComponent['currentDoc']) => {
      ...
      parentFixture
          .detectChanges();      // Run change detection to propagate the new doc to `DocViewer`.
        ...
        spyOn(targetEl, 'querySelector').and.callFake((selector: string) => {
          ...
          return elem &&
              Object.defineProperties(
                  elem, {innerText: {value: ''}, textContent: {value: 'Text Content'}});
        });
  ...
  describe('#render()', () => {
    ...
    beforeEach(() => {
      const elementsLoader =
          TestBed.inject(ElementsLoader) as Partial<ElementsLoader>as MockElementsLoader;
      loadElementsSpy =
          elementsLoader.loadContainedCustomElements.and.callFake(() => of(undefined));
    ...
    describe('(contents, title, ToC)', () => {
      ...
      it('should display the document contents', async () => {
        const contents = htmlFromStringKnownToSatisfyTypeContract(
            '<h1>Hello,</h1> <div>world!</div>', 'constant HTML');
      ...
      it('should add the "noindex" meta tag if the document is 404', async () => {
        ...
        expect(TestBed.inject(Meta).addTag)
            .toHaveBeenCalledWith({name: 'robots', content: 'noindex'});
      });
    ...
    describe('(embedding components)', () => {
      ...
      it('should unsubscribe from the previous "embed" observable when unsubscribed from', () => {
        ...
        const renderObservable =
            docViewer.render({contents: htmlEscape('Some content'), id: 'foo'});
    ...
    describe('(swapping views)', () => {
      ...
      it('should unsubscribe from the previous "swap" observable when unsubscribed from', () => {
        ...
        const renderObservable =
            docViewer.render({contents: htmlEscape('Hello, world!'), id: 'foo'});
        const subscription = renderObservable.subscribe();
    ...
    describe('(on error) should clean up, log the error and recover', () => {
      ...
      it('when `prepareTitleAndTocSpy()` fails', async () => {
        ...
        expect(logger.output.error[0][0].message)
            .toEqual(`[DocViewer] Error preparing document 'foo': ${error.stack}`);
        expect(TestBed.inject(Meta).addTag)
            .toHaveBeenCalledWith({name: 'robots', content: 'noindex'});
      });
      ...
      it('when `swapViews()` fails', async () => {
        ...
        expect(logger.output.error).toEqual([[jasmine.any(Error)]]);
        expect(logger.output.error[0][0].message)
            .toEqual(`[DocViewer] Error preparing document 'qux': ${error.stack}`);
        expect(TestBed.inject(Meta).addTag)
            .toHaveBeenCalledWith({name: 'robots', content: 'noindex'});
      });
      ...
      it('when something fails with non-Error', async () => {
        ...
        expect(logger.output.error).toEqual([[jasmine.any(Error)]]);
        expect(logger.output.error[0][0].message)
            .toEqual(`[DocViewer] Error preparing document 'qux': ${error}`);
        expect(TestBed.inject(Meta).addTag)
            .toHaveBeenCalledWith({name: 'robots', content: 'noindex'});
      });
    });
        ...
        [true, false].forEach(noAnimations => {
          describe(`(.${NO_ANIMATIONS}: ${noAnimations})`, () => {
            ...
            if (animationsEnabled && !noAnimations) {
              ...
              it('should abort swapping if the returned observable is unsubscribed from',
                 async () => {
