...
describe('SwUpdatesService', () => {
    ...
    injector = Injector.create({
      providers: [
        ...
        {provide: SwUpdate, useFactory: () => new MockSwUpdate(isSwUpdateEnabled), deps: []}, {
          provide: SwUpdatesService,
          deps: [ApplicationRef, ErrorHandler, LocationService, Logger, SwUpdate]
        }
      ]
    });
  ...
  describe('when enabled', () => {
    ...
    it('should keep periodically checking for updates even after one is available/activated',
       fakeAsync(run(() => {
         appRef.isStable.next(true);
         swu.checkForUpdate.calls.reset();
    ...
    describe('and then disabled', () => {
      ...
      it('should not schedule a new check for update (after activating an update)',
         fakeAsync(run(() => {
           appRef.isStable.next(true);
           expect(swu.checkForUpdate).toHaveBeenCalled();
      ...
      describe('and then enabled again', () => {
        ...
        it('should start scheduling new checks for updates (after activating an update) again',
           fakeAsync(run(() => {
             appRef.isStable.next(true);
             service.disable();
             service.enable();
...
class MockSwUpdate {
  ...
  activateUpdate =
      jasmine.createSpy('MockSwUpdate.activateUpdate').and.callFake(() => Promise.resolve());

  checkForUpdate =
      jasmine.createSpy('MockSwUpdate.checkForUpdate').and.callFake(() => Promise.resolve());
